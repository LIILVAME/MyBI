  ...
  >
    ...
    <div class="flex-1 flex flex-col">
      ...
      <div class="flex items-start justify-between mb-2 sm:mb-3">
        <div class="flex-1 min-w-0 pr-2">
          <h3 class="text-sm sm:text-base lg:text-lg font-semibold text-gray-900 mb-1 truncate">{{ property.name }}</h3>
          <p class="text-xs text-gray-500 mb-1 truncate">{{ property.city }}</p>
          <p class="text-xs text-gray-400 mb-2 line-clamp-2 hidden sm:block">{{ property.address }}</p>
          <span 
      ...
      <div class="mt-2 sm:mt-4 mb-2 sm:mb-4 flex-1 flex flex-col">
        <div class="flex items-center justify-between mb-2">
          ...
          <p class="text-base sm:text-lg lg:text-xl font-bold text-gray-900 whitespace-nowrap">{{ formatCurrency(property.rent) }}</p>
        </div>
        ...
        <div class="flex-1 flex flex-col">
          ...
          <div v-else class="border-t border-gray-100 pt-2 sm:pt-4 mt-2 sm:mt-4 min-h-[60px] sm:min-h-[80px] flex items-center justify-center">
            <p class="text-gray-400 text-xs sm:text-sm italic text-center px-2">{{ $t('tenants.noTenants') }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="mt-auto pt-2 sm:pt-4 border-t border-gray-100 flex items-center gap-1.5 sm:gap-2">
      <button
        ...
        class="flex-1 px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm font-medium text-primary-600 bg-primary-50 rounded-lg hover:bg-primary-100 transition-colors"
      >
        <svg class="w-3 h-3 sm:w-4 sm:h-4 inline sm:mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <button
        ...
        class="flex-1 px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm font-medium text-red-600 bg-red-50 rounded-lg hover:bg-red-100 transition-colors"
      >
        <svg class="w-3 h-3 sm:w-4 sm:h-4 inline sm:mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
